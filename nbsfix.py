...
print(
    "本脚本依照以下开源协议发布，请阅读后按下回车键表示同意、同时按下 `Ctrl` 和 `C` 键表示不同意并退出本脚本。"
)
print(
    "The Script is Licensed under the Following Open Source License. Press `Enter` to Agree, `Ctrl+C` to Cancel and Quit."
)
input(COPYRIGNT)
...
def main():
    ...
    if ori_roaming_path == ansi_roeaming_path:
        ...
        return 0
    if not (
        opth := (ori_roaming_path / "Minecraft_Note_Block_Studio").resolve()
    ).exists():
        print("!!! 请先运行一遍 NBS | You Might Never Run NBS Before !!!")
        print("如果你没有运行过 NBS，那是怎么知道需要修复路径问题的？")
        print(
            "Please Run NBS at Least Once "
            "Before You Want to Fix Your %APPDATA% Path Problem."
        )
        return 0
    if (
        epth := ansi_roeaming_path / "Minecraft_Note_Block_Studio"
    ).exists() and epth.is_symlink():
        print("!!! 请不要重复运行本修复脚本 | Please Don't Run This Script Again !!!")
        print("已经修过了，就别再修了，唧唧复唧唧，木兰都能唧。")
        print(
            "If You Have Already Fixed This, Don't Run the Script Again; "
            "If You Havn't, NBS will Not Work Inproperly."
        )
        return 0
    ...
    epth.symlink_to(opth, target_is_directory=True)
